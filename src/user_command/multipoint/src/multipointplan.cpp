...
bool isInState(uint16_t state, uint16_t in)
{
    uint16_t l = state - 100;
    uint16_t h = state + 100;
    return l < in && in < h;
}
void rc_cb(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    if (!isInState(RC_EIGHT_DOWN, rc_eight_cur) && rc_init)
    {
        std::cout << "ch_8: " << rc_eight_cur << std::endl;
        return;
    }
    else
        rc_init = false;
    ...
    if (isInState(RC_EIGHT_MIDDLE, rc_eight_cur) && rc_eight_pre == RC_EIGHT_DOWN)
    {
    ...
    if (isInState(RC_EIGHT_UP, rc_eight_cur) && rc_eight_pre == RC_EIGHT_MIDDLE)
    {
    ...
    if (isInState(RC_EIGHT_MIDDLE, rc_eight_cur) && rc_eight_pre == RC_EIGHT_UP)
    {
    ...
    if (isInState(RC_EIGHT_DOWN, rc_eight_cur) && rc_eight_pre == RC_EIGHT_MIDDLE)
    {
        quadrotor_msgs::TakeoffLand takeoff_msg;
