...
通过 `world_transform` 功能，无论使用哪个 IMU 进行里程计计算，输出的坐标系都可以与飞控 IMU 坐标系对齐。

### 切换 IMU 配置

编辑 `config/mid360.yaml` 文件来切换 IMU 配置：
...
mapping:
    # IMU-LiDAR 外参（雷达相对飞控 IMU 的位置和旋转，包含斜装角度）
