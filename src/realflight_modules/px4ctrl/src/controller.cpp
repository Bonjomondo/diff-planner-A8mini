                                                     ...
                                                     double voltage)
{
    ...
    if (des.v(2) < -3.0)
        ROS_WARN("[px4ctrl] Desired z-Velocity = %6.3fm/s, < -3.0m/s, which is dangerous "
    ...
    translational_acc =
        (Gravity +
         computeLimitedTotalAccFromThrustForce(translational_acc - Gravity, 1.0))
            .eval();

    // wmywmy
    minimumSingularityFlatWithDrag(param.mass,
                                   param.gra,
                                   des.v,
                                   des.a,
                                   des.j,
    ...
    u.thrust = computeDesiredCollectiveThrustSignal(odom.q,
                                                    odom.v,
                                                    total_des_acc,
                                                    param,
                                                    voltage);
    ...
    if (param.max_angle > 0)
    {
        ...
        if (rot_ang > param.max_angle) // Exceed the angle limit
        {
            Eigen::Vector3d limited_z_B =
                Eigen::AngleAxisd(param.max_angle, rot_axis) * Eigen::Vector3d::UnitZ();
...
void Controller::minimumSingularityFlatWithDrag(const double mass,
                                                ...
                                                double &thrust) const
{
    // Drag effect parameters (Drag may cause larger tracking error in aggressive flight
    // during our tests) dv >= dh is required dv is the rotor drag effect in vertical
    // direction, typical value is 0.35 dh is the rotor drag effect in horizontal
    // direction, typical value is 0.25 cp is the second-order drag effect, typical valye
    // is 0.01 const double dh = 0.10; const double dv = 0.23; const double cp = 0.01;
                                                     ...
                                                     double voltage)
{
    ...
    computeFlatInput(total_des_acc,
                     des.j,
                     des.yaw,
                     des.yaw_rate,
                     odom.q,
...
Eigen::Vector3d Controller::computeRobustBodyXAxis(const Eigen::Vector3d &x_B_prototype,
                                               ...
                                               const Parameter_t &param)
{
    ...
    // x acceleration
    double x_pos_error =
        std::isnan(des.p(0)) ? 0.0 : std::max(std::min(des.p(0) - odom.p(0), 1.0), -1.0);
    double x_vel_error =
        std::max(std::min((des.v(0) + Kp(0) * x_pos_error) - odom.v(0), 1.0), -1.0);
    acc_error(0) = Kv(0) * x_vel_error;

    // y acceleration
    double y_pos_error =
        std::isnan(des.p(1)) ? 0.0 : std::max(std::min(des.p(1) - odom.p(1), 1.0), -1.0);
    double y_vel_error =
        std::max(std::min((des.v(1) + Kp(1) * y_pos_error) - odom.v(1), 1.0), -1.0);
    acc_error(1) = Kv(1) * y_vel_error;

    // z acceleration
    double z_pos_error =
        std::isnan(des.p(2)) ? 0.0 : std::max(std::min(des.p(2) - odom.p(2), 1.0), -1.0);
    double z_vel_error =
        std::max(std::min((des.v(2) + Kp(2) * z_pos_error) - odom.v(2), 1.0), -1.0);
...
Eigen::Vector3d Controller::computeLimitedAngularAcc(
...
double Controller::computeDesiredCollectiveThrustSignal(const Eigen::Quaterniond &est_q,
                                                        ...
                                                        double voltage)
{
    ...
    {
        normalized_thrust =
            thr_scale_compensate * AccurateThrustAccMapping(des_acc_norm, voltage, param);
    }
    else
    {
                                            ...
                                            const Parameter_t &param) const
{
    ...
    double c =
        -(param.mass * des_acc_z) / (param.thr_map.K1 * pow(voltage, param.thr_map.K2));
